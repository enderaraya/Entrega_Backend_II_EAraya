...
export const getCartById = async (req, res, next) => {
    try {
        ...
        if (!response) res.status(404).json({ msg: "Este carrito no existe o no se pudo encontrar" });
...
export const deleteCart = async (req, res, next) => {
    try {
        ...
        if (!cartDel) res.status(404).json({ msg: "No se pudo borrar el carrito" });
        else res.status(200).json({ msg: `El carrito (${cid}) fue borrado correctamente` });
    } catch (error) {
        next(error.message);
    }
};
...
export const removefromCart = async (req, res, next) => {
    try {
        ...
        if (!delProdToUserCart) res.json({ msg: "No se pudo borrar el producto del carrito" });
        else res.json({ msg: `${pid} fue removido exitosamente del carrito` });
    } catch (error) {
        next(error.message);
    }
};
...
export const clearCart = async (req, res, next) => {
    try {
        ...
        if (!clearCart) res.json({ msg: "No se pudo borrar el carrito" });
...
export const purchase = async (req, res) => {
    ...
    try {
        ...
        if (!cart) {
            return res.status(400).send("Carrito inexistente");
        }
        ...
        if (ticketResponse) {
            ...
            return res.status(400).send("Error al generar el ticket");
        }
    } catch (error) {
        console.error(error);
